...
async function sendTrace(phoenixUrl, projectName, input, output, endTime) {
    ...
    if (output.usage?.input != null)
        attributes["llm.token_count.prompt"] = output.usage.input;
    if (output.usage?.output != null)
        attributes["llm.token_count.completion"] = output.usage.output;
    if (output.usage?.total != null)
        attributes["llm.token_count.total"] = output.usage.total;
    if (output.usage?.cacheRead != null)
        attributes["llm.token_count.cache_read"] = output.usage.cacheRead;
    if (output.usage?.cacheWrite != null)
        attributes["llm.token_count.cache_write"] = output.usage.cacheWrite;
    const body = {
        data: [
            {
    ...
    const res = await fetch(`${phoenixUrl}/v1/projects/${encodeURIComponent(projectName)}/spans`, {
        method: "POST",
